...
<head>
    ...
    <style>
        ...
        .topbar {
            ...
            padding:14px 30px; border-bottom:1px solid #e5e7eb;
        ...
        .card-wrap {
            ...
            border:1px solid #e5e7eb; overflow:hidden; margin:28px 30px;
        }
        .card-top {
        ...
        .admin-table tbody td {
            padding:16px 20px; border-bottom:1px solid #f3f4f6; vertical-align:middle;
        }
        ...
        .action-btn.warning  { border-color:#fcd34d; }
        .action-btn.warning:hover { background:#fef9c3; }
        .action-btn:disabled { opacity:.35; cursor:not-allowed; }

        .badge-role-super { background:#0b1533; color:#fff; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; }
        .badge-role-kasir { background:#e5e7eb; color:#374151; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; }
        .badge-aktif      { background:#dcfce7; color:#16a34a; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; }
        .badge-nonaktif   { background:#fee2e2; color:#dc2626; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:600; }

        /* Modal password result */
        .password-box {
            background:#f9fafb; border:1px solid #e5e7eb; border-radius:12px;
            padding:16px; text-align:center; margin:12px 0;
            font-size:24px; font-weight:700; letter-spacing:3px; color:#0b1533;
        ...
        .toast-msg {
            background:#0b1533; color:#fff; padding:12px 20px;
...
<div class="admin-layout">
    ...
    <div class="admin-content">
        ...
        <div style="margin:12px 30px;" class="alert alert-danger alert-dismissible fade show d-flex align-items-center gap-2">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            {{ session('error') }}
            <button type="button" class="btn-close ms-auto" data-bs-dismiss="alert"></button>
        </div>
        @endif
        ...
        <div class="card-wrap">
            <div class="card-top">
                <div>
                    <div class="card-title">Daftar Admin & Kasir</div>
            ...
            <div class="empty-state">
                <p style="font-size:15px;font-weight:600;color:#6b7280;">Belum ada admin terdaftar.</p>
            </div>
            @else
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight:600; color:#0b1533;">{{ $admin->name ?? $admin->username }}</td>
                    <td><code style="font-size:12px; color:#6b7280;">{{ $admin->username }}</code></td>
                    <td style="color:#6b7280; font-size:13px;">{{ $admin->email ?? '—' }}</td>
                    <td>
                        @if($admin->role === 'manager')
                            <span class="badge-role-super">Manager</span>
                        @else
                            <span class="badge-role-kasir">Kasir</span>
                        @endif
                    </td>
